...
description: Fetch and inspect recent local server logs in repos that use openlogs or the \`ol\` CLI. Use when a user asks what happened in the server, wants recent dev-server output, needs startup errors or stack traces, or asks you to check backend logs from \`.openlogs/latest.txt\` or \`openlogs tail\`.
---
...
- Run \`openlogs tail -n 200\` to inspect the latest text log.
- Use \`ol tail -n 200\` if the short alias is preferred.
- Read \`.openlogs/latest.txt\` directly when file access is simpler than spawning the command.
...
2. If that fails, try \`ol tail -n 200\`.
3. If the CLI is unavailable but the workspace is accessible, read \`.openlogs/latest.txt\`.
4. If the log directory is missing, check whether the server was started with \`openlogs <command>\` or \`ol <command>\`.
5. If it was not, tell the user to relaunch the server through openlogs, then inspect the resulting logs.
